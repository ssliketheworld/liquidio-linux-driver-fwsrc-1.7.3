...
void lio_cn6xxx_enable_error_reporting(struct octeon_device *oct)
{
	...
	OCTEON_READ_PCI_CONFIG(oct, CN6XXX_PCIE_DEVCTL, &val);
	if (val & 0x000c0000) {
	...
	OCTEON_WRITE_PCI_CONFIG(oct, CN6XXX_PCIE_DEVCTL, val);
}
			       ...
			       enum octeon_pcie_mps mps)
{
	...
	cavium_write_once64(r64, lio_pci_readq(oct,
				      CN6XXX_DPI_SLI_PRTX_CFG(oct->pcie_port)));
	cavium_write_once64(r64, cavium_read_once64(r64) | (mps << 4));
	lio_pci_writeq(oct, cavium_read_once64(r64),
		       CN6XXX_DPI_SLI_PRTX_CFG(oct->pcie_port));
}
				...
				enum octeon_pcie_mrrs mrrs)
{
	...
	cavium_write_once64(r64, octeon_read_csr64(oct, CN6XXX_SLI_S2M_PORTX_CTL
						(oct->pcie_port)));
	cavium_write_once64(r64, cavium_read_once64(r64) | mrrs);
	octeon_write_csr64(oct, CN6XXX_SLI_S2M_PORTX_CTL(oct->pcie_port),
			   cavium_read_once64(r64));
...
{
	int i;
	u32 mask, loop = CAVIUM_TICKS_PER_SEC;
			  ...
			  int valid)
{
	...
	if (valid == 0) {
		cavium_write_once64(bar1, lio_pci_readq(oct, CN6XXX_BAR1_REG
							(idx, oct->pcie_port)));
...
{
	u32 new_idx = OCTEON_READ32(iq->inst_cnt_reg);
				 ...
				 u8 unused UNUSED)
{
	...
	/* Enable Interrupt */
	OCTEON_WRITE64(cn6xxx->intr_enb_reg64, mask);
}
				  ...
				  u8 unused UNUSED)
{
	...
	/* Disable Interrupts */
	OCTEON_WRITE64(cn6xxx->intr_enb_reg64, 0);
	...
	for (oq_no = 0; oq_no < MAX_OCTEON_OUTPUT_QUEUES(oct); oq_no++) {
		...
		if (pkt_count) {
			...
			if (droq->ops.poll_mode) {
				...
				cavium_interrupt_spin_unlock(&cn6xxx->lock_for_droq_int_enb_reg);
			}
		}
	}
...
cvm_intr_return_t lio_cn6xxx_process_interrupt_regs(void *dev)
{
				  ...
				  struct octeon_reg_list *reg_list)
{
	...
	reg_list->pci_win_wr_addr_hi =
		(u32 cavium_iomem *)(bar0_pciaddr + CN6XXX_WIN_WR_ADDR_HI);
	reg_list->pci_win_wr_addr_lo =
		(u32 cavium_iomem *)(bar0_pciaddr + CN6XXX_WIN_WR_ADDR_LO);
	reg_list->pci_win_wr_addr =
		(u64 cavium_iomem *)(bar0_pciaddr + CN6XXX_WIN_WR_ADDR64);

	reg_list->pci_win_rd_addr_hi =
		(u32 cavium_iomem *)(bar0_pciaddr + CN6XXX_WIN_RD_ADDR_HI);
	reg_list->pci_win_rd_addr_lo =
		(u32 cavium_iomem *)(bar0_pciaddr + CN6XXX_WIN_RD_ADDR_LO);
	reg_list->pci_win_rd_addr =
		(u64 cavium_iomem *)(bar0_pciaddr + CN6XXX_WIN_RD_ADDR64);

	reg_list->pci_win_wr_data_hi =
		(u32 cavium_iomem *)(bar0_pciaddr + CN6XXX_WIN_WR_DATA_HI);
	reg_list->pci_win_wr_data_lo =
		(u32 cavium_iomem *)(bar0_pciaddr + CN6XXX_WIN_WR_DATA_LO);
	reg_list->pci_win_wr_data =
		(u64 cavium_iomem *)(bar0_pciaddr + CN6XXX_WIN_WR_DATA64);

	reg_list->pci_win_rd_data_hi =
		(u32 cavium_iomem *)(bar0_pciaddr + CN6XXX_WIN_RD_DATA_HI);
	reg_list->pci_win_rd_data_lo =
		(u32 cavium_iomem *)(bar0_pciaddr + CN6XXX_WIN_RD_DATA_LO);
	reg_list->pci_win_rd_data =
		(u64 cavium_iomem *)(bar0_pciaddr + CN6XXX_WIN_RD_DATA64);
...
int lio_setup_cn66xx_octeon_device(struct octeon_device *oct)
{
	...
	cavium_spin_lock_init(&cn6xxx->lock_for_droq_int_enb_reg);
				    ...
				    struct octeon_config *conf6xxx)
{
	if (CFG_GET_IQ_MAX_Q(conf6xxx) > CN6XXX_MAX_INPUT_QUEUES) {
		lio_dev_err(oct, "%s: Num IQ (%d) exceeds Max (%d)\n",
			    __CVM_FUNCTION__, CFG_GET_IQ_MAX_Q(conf6xxx),
	...
	if (CFG_GET_OQ_MAX_Q(conf6xxx) > CN6XXX_MAX_OUTPUT_QUEUES) {
		lio_dev_err(oct, "%s: Num OQ (%d) exceeds Max (%d)\n",
			    __CVM_FUNCTION__, CFG_GET_OQ_MAX_Q(conf6xxx),
	...
	if ((CFG_GET_OQ_INFO_PTR(conf6xxx) != OCTEON_OQ_INFOPTR_MODE) ||
