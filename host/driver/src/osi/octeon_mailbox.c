...
int octeon_mbox_read(struct octeon_mbox *mbox)
{
	...
	cavium_interrupt_spin_lock(&mbox->lock);

	msg.u64 = OCTEON_READ64(mbox->mbox_read_reg);

	if ((msg.u64 == OCTEON_PFVFACK) || (msg.u64 == OCTEON_PFVFSIG)) {
		cavium_interrupt_spin_unlock(&mbox->lock);
		return 0;
	}
		...
		if (mbox->state & OCTEON_MBOX_STATE_RESPONSE_RECEIVING) {
			...
			if ((mbox->state & OCTEON_MBOX_STATE_IDLE) &&
				...
				if ((mbox->state &
					...
					OCTEON_WRITE64(mbox->mbox_read_reg,
						       OCTEON_PFVFERR);
		      ...
		      struct octeon_mbox_cmd *mbox_cmd)
{
	...
	cavium_interrupt_spin_lock_irqsave(&mbox->lock, flags);

	if ((mbox_cmd->msg.s.type == OCTEON_MBOX_RESPONSE) &&
	    !(mbox->state & OCTEON_MBOX_STATE_REQUEST_RECEIVED)) {
		cavium_interrupt_spin_unlock_irqrestore(&mbox->lock, flags);
	...
	if (mbox_cmd->msg.s.type == OCTEON_MBOX_REQUEST) {
		cavium_memcpy(&mbox->mbox_resp, mbox_cmd,
	...
	while (OCTEON_READ64(mbox->mbox_write_reg) != OCTEON_PFVFSIG) {
		cavium_sleep_timeout(LIO_MBOX_WRITE_WAIT_TIME);
	...
	if (ret == OCTEON_MBOX_STATUS_SUCCESS) {
		OCTEON_WRITE64(mbox->mbox_write_reg, mbox_cmd->msg.u64);
		for (i = 0; i < (u32)(mbox_cmd->msg.s.len - 1); i++) {
			count = 0;
			while (OCTEON_READ64(mbox->mbox_write_reg) !=
				OCTEON_PFVFACK) {
				cavium_sleep_timeout(
						LIO_MBOX_WRITE_WAIT_TIME);
				if (OCTEON_READ64(mbox->mbox_write_reg) ==
				    OCTEON_PFVFSIG) {
					ret = OCTEON_MBOX_STATUS_FAILED;
					break;
				}
			...
			if (ret == OCTEON_MBOX_STATUS_SUCCESS)
				OCTEON_WRITE64(mbox->mbox_write_reg,
					       mbox_cmd->data[i]);
			else
				break;
		}
				   ...
				   struct octeon_mbox_cmd *mbox_cmd)
{
	...
	case OCTEON_VF_ACTIVE:
		lio_dev_dbg(oct, "got vfactive sending data back\n");
		...
		cavium_memcpy(mbox_cmd->msg.s.params,
			      (u8 *)&oct->pfvf_hsword, 6);
	...
	case OCTEON_VF_FLR_REQUEST:
		lio_dev_info(oct,
			     ...
			     mbox->q_no);
		cavium_pci_reset_function(
			oct->sriov_info.dpiring_to_vfpcidev_lut[mbox->q_no]);
		break;

	case OCTEON_CORES_CRASHED:
		lio_dev_err(oct, "ERROR: Octeon core(s) crashed or got stuck!\n");
		break;

	case OCTEON_PF_CHANGED_VF_MACADDR:
		octeon_pf_changed_vf_macaddr(oct, mbox_cmd->msg.s.params);
		break;

	case OCTEON_PF_SET_OR_CLEARED_VF_VLAN:
		octeon_pf_set_or_cleared_vf_vlan(oct,
						 mbox_cmd->msg.s.params[0]);
		break;

	case OCTEON_GET_VF_STATS:
		lio_dev_dbg(oct, "Got VF stats request. Sending data back\n");
		mbox_cmd->msg.s.type = OCTEON_MBOX_RESPONSE;
		mbox_cmd->msg.s.resp_needed = 1;
		mbox_cmd->msg.s.len = 1 + sizeof(struct oct_vf_stats) / sizeof(u64);
...
int octeon_mbox_process_message(struct octeon_mbox *mbox)
{
	...
	if (mbox->state & OCTEON_MBOX_STATE_ERROR) {
		if (mbox->state & (OCTEON_MBOX_STATE_RESPONSE_PENDING |
				   OCTEON_MBOX_STATE_RESPONSE_RECEIVING)) {
			cavium_memcpy(&mbox_cmd, &mbox->mbox_resp,
				      sizeof(struct octeon_mbox_cmd));
			mbox->state = OCTEON_MBOX_STATE_IDLE;
			OCTEON_WRITE64(mbox->mbox_read_reg, OCTEON_PFVFSIG);
			cavium_interrupt_spin_unlock_irqrestore(&mbox->lock, flags);
	...
	if (mbox->state & OCTEON_MBOX_STATE_REQUEST_RECEIVED) {
		cavium_memcpy(&mbox_cmd, &mbox->mbox_req,
			      sizeof(struct octeon_mbox_cmd));
		if (!mbox_cmd.msg.s.resp_needed) {
			mbox->state &= ~OCTEON_MBOX_STATE_REQUEST_RECEIVED;
			if (!(mbox->state &
...
int
octeon_mbox_cancel(struct octeon_device *oct, int q_no)
{
	...
	mbox->state = OCTEON_MBOX_STATE_IDLE;
	cavium_memset(mbox_cmd, 0, sizeof(*mbox_cmd));
	OCTEON_WRITE64(mbox->mbox_read_reg, OCTEON_PFVFSIG);
	cavium_interrupt_spin_unlock_irqrestore(&mbox->lock, flags);
