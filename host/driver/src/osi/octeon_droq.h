...
#ifdef CAVIUM_BYTE_ALLOC_RXBUFS 
/*
 * CAVIUM_BYTE_ALLOC_RXBUFS
 * This describes driver behavior whereby the rx buffers are discrete, 
 * byte buffers, as opposed to the default Linux driver behavior,
 * which allocates RX buffers as pages (2 per page).
 * This was originally represented by 'CONFIG_PAGE_ALLOCATIONS'.
 */
#endif
...
static inline struct octeon_recv_info *octeon_alloc_recv_info(int extra_bytes)
{
	...
	buf = cavium_kmalloc(OCT_RECV_PKT_SIZE + OCT_RECV_INFO_SIZE +
...
struct octeon_droq {
	 ...
	 */
	void cavium_volatile_t cavium_iomem *pkts_credit_reg;

	/** Pointer to the mapped packet sent register.
	 * Octeon writes the number of packets DMA'ed to host memory
	 * in this register.
	 */
	void cavium_iomem *pkts_sent_reg;

	struct cavium_list_head dispatch_list;
	...
	struct cavium_napi_struct napi;

	u32 cpu_id;

	cavium_call_single_data_t csd;
};
...
u32 octeon_droq_refill(struct octeon_device *octeon_dev,
		       struct octeon_droq *droq);
int octeon_retry_droq_refill(struct octeon_droq *droq);

#ifdef CAVIUM_BYTE_ALLOC_RXBUFS 
#define tx_buffer_free recv_buffer_free
#endif
