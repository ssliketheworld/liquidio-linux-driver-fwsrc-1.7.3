...
struct octeon_instr_queue {
	...
	/** A spinlock to protect access to the input ring.*/
	cavium_spintrylock_t iq_flush_running_lock;
	...
	void cavium_iomem *doorbell_reg;
...
#define COMPLETION_WORD_INIT    0xffffffffffffffffULL
	...
	unsigned long done;
#define CALLER_DONE_BIT		0
#define ORDERED_PROCESS_DONE_BIT	1
	u32 sc_status;
	struct cavium_completion complete;
};

#define LIO_SC_MAX_TMO_MS       60000   /* max timeout (in milli sec) for soft request */
...
struct octeon_sc_buffer_pool {
	...
	struct cavium_list_head head;

	/** A lock for this response list */
	cavium_spinlock_t lock;

	cavium_atomic_t alloc_buf_count;
};
...
int octeon_free_sc_processed_done_list(struct octeon_device *oct);
