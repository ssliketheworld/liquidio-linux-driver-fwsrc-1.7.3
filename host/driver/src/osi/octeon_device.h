...
#define  OCTEON_CN23XX_PCIID_VF       0x9712177d
#define  OCTEON_CN23XX_PCIID_HVF      0xEF00177d
...
#define  OCTEON_CN23XX_HVF_VID	      0xEF00
...
struct octeon_reg_list {
	u32 cavium_iomem *pci_win_wr_addr_hi;
	u32 cavium_iomem *pci_win_wr_addr_lo;
	u64 cavium_iomem *pci_win_wr_addr;

	u32 cavium_iomem *pci_win_rd_addr_hi;
	u32 cavium_iomem *pci_win_rd_addr_lo;
	u64 cavium_iomem *pci_win_rd_addr;

	u32 cavium_iomem *pci_win_wr_data_hi;
	u32 cavium_iomem *pci_win_wr_data_lo;
	u64 cavium_iomem *pci_win_wr_data;

	u32 cavium_iomem *pci_win_rd_data_hi;
	u32 cavium_iomem *pci_win_rd_data_lo;
	u64 cavium_iomem *pci_win_rd_data;
};
...
struct octeon_fn_list {
	...
	cvm_intr_return_t (*process_interrupt_regs)(void *);
...
struct octdev_props {
	...
	int    napi_enabled;
	int    gmxport;
	struct cavium_net_device *netdev;
};
...
struct octeon_sriov_info {
	...
	struct cavium_pci_device *dpiring_to_vfpcidev_lut[MAX_POSSIBLE_VFS];
...
struct octeon_ioq_vector {
	...
	cavium_cpumask		affinity_mask;
	u32			ioq_num;
};

struct lio_vf_rep_list {
	int num_vfs;
	struct cavium_net_device *ndev[CN23XX_MAX_VFS_PER_PF];
};
...
struct octeon_device {
	...
	cavium_spinlock_t pci_win_lock;

	/** Lock for memory accesses */
	cavium_spinlock_t mem_access_lock;
...
#define LIO_FLAG_MSIX_ENABLED                 (u32)(1 << 2)
	...
	cavium_spinlock_t cmd_resp_wqlock;
	u32 cmd_resp_state;
...
#define OCTEON_UBOOT_BUFFER_SIZE 512
	...
	cavium_atomic_t *adapter_refcount; /* reference count of adapter */

	cavium_atomic_t *adapter_fw_state; /* per-adapter, lio_fw_state */
...
#  if (RHEL_RELEASE_CODE >= RHEL_RELEASE_VERSION(6, 0)) && (RHEL_RELEASE_CODE < RHEL_RELEASE_VERSION(7, 0))
	struct cavium_workqueue *mbox_wq;
#  endif
#endif
	struct lio_vf_rep_list vf_rep_list;
	struct cavium_devlink *devlink;
	int eswitch_mode;

	cavium_atomic_t sc_zombie_cnt;
...
static inline int OCTEON_CN6XXX(struct octeon_device *oct)
{
	return (oct->chip_id == OCTEON_CN66XX ||
...
#define   octeon_write_csr(oct_dev, reg_off, value) \
		OCTEON_WRITE32((oct_dev)->mmio[0].hw_addr + (reg_off), value)

#define   octeon_write_csr64(oct_dev, reg_off, val64) \
		OCTEON_WRITE64((oct_dev)->mmio[0].hw_addr + (reg_off), val64)

#define   octeon_read_csr(oct_dev, reg_off)         \
		OCTEON_READ32((oct_dev)->mmio[0].hw_addr + (reg_off))

#define   octeon_read_csr64(oct_dev, reg_off)         \
		OCTEON_READ64((oct_dev)->mmio[0].hw_addr + (reg_off))
