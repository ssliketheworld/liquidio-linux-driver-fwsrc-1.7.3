...
u32 octeon_droq_check_hw_for_pkts(struct octeon_droq *droq)
{
	...
	if (last_count)
		cavium_atomic_add(last_count, &droq->pkts_pending);
				 ...
				 struct octeon_droq *droq)
{
	...
	if ((oct == NULL) || (droq == NULL) || (droq->recv_buf_list == NULL))
		return;
	...
	for (i = 0; i < droq->max_count; i++) {
		...
		if (droq->recv_buf_list[i].buffer) {
			if (droq->desc_ring) {
				lio_unmap_ring(oct->pci_dev, (u64)
...
int octeon_delete_droq(struct octeon_device *oct, u32 q_no)
{
	...
	lio_dev_dbg(oct, "%s[%d]\n", __CVM_FUNCTION__, q_no);

	octeon_droq_destroy_ring_buffers(oct, droq);
	cavium_vfree(droq->recv_buf_list);
		     ...
		     void *app_ctx)
{
	...
	int numa_node = cavium_droq_to_node(oct, q_no);
	...
	droq->desc_ring = lio_dma_alloc(oct, "droq", q_no, desc_ring_size,
					(cavium_dma_addr_t *)&droq->desc_ring_dma);
	if (!droq->desc_ring) {
		lio_dev_err(oct,
	...
	droq->recv_buf_list = (struct octeon_recv_buffer *)
			      cavium_vzalloc_node(droq->max_count *
						  OCT_DROQ_RECVBUF_SIZE,
						  numa_node);
	if (!droq->recv_buf_list)
		droq->recv_buf_list = (struct octeon_recv_buffer *)
				      cavium_vzalloc(droq->max_count *
						     OCT_DROQ_RECVBUF_SIZE);
	if (!droq->recv_buf_list) {
...
#else
			buf = recv_buffer_alloc(octeon_dev,
						droq->q_no, droq->buffer_size);
#endif
...
int octeon_retry_droq_refill(struct octeon_droq *droq)
{
	...
	pkts_credit = OCTEON_READ32(droq->pkts_credit_reg);
				 ...
				 u32 pkts_to_process)
{
	...
	for (pkt = 0; pkt < pkt_count; pkt++) {
		u32 pkt_len = 0;
		struct cavium_netbuf *nicbuf = NULL;
#ifndef CAVIUM_BYTE_ALLOC_RXBUFS
		struct octeon_skb_page_info *pg_info;
		void *buf;
#endif
	...
	cavium_atomic_sub(retval, &droq->pkts_pending);

	if (droq->refill_count >= droq->refill_threshold &&
	    OCTEON_READ32(droq->pkts_credit_reg) < CN23XX_SLI_DEF_BP) {
			      ...
			      struct octeon_droq *droq, u32 budget)
{
	...
	while (total_pkts_processed < budget) {
		...
		pkts_available = CVM_MIN((budget - total_pkts_processed),
				     (u32)(cavium_atomic_read(&droq->pkts_pending)));
...
octeon_enable_irq(struct octeon_device *oct, u32 q_no)
{
	...
	case OCTEON_CN68XX: {
		...
		cavium_interrupt_spin_lock_irqsave
		...
		cavium_interrupt_spin_unlock_irqrestore
			(&cn6xxx->lock_for_droq_int_enb_reg, flags);
	}
		break;
	...
	case OCTEON_CN23XX_VF_VID: {
		...
		break;
	default:
		lio_dev_err(oct, "%s Unknown Chip\n", __CVM_FUNCTION__);
		return 1;
	}
	return 0;
}
			     ...
			     struct octeon_droq_ops *ops)
{
	...
	droq = oct->droq[q_no];
	cavium_memcpy(&droq->ops, ops, sizeof(struct octeon_droq_ops));
		       ...
		       u32 desc_size, void *app_ctx)
{
	...
	droq = cavium_vmalloc_node(sizeof(*droq), numa_node);
	if (!droq)
		droq = cavium_vmalloc(sizeof(*droq));
	if (!droq)
		goto create_droq_fail;

	cavium_memset(droq, 0, sizeof(struct octeon_droq));
