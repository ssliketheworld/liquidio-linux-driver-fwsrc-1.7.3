...
static cavium_atomic_t adapter_refcounts[MAX_OCTEON_DEVICES];
static cavium_atomic_t adapter_fw_states[MAX_OCTEON_DEVICES];

static u32 octeon_device_count;
/* locks device array (i.e. octeon_device[]) */
static cavium_spinlock_t octeon_devices_lock;
...
void octeon_init_device_list(int conf_type)
{
	...
	cavium_spin_lock_init(&octeon_devices_lock);
}
...
char *lio_get_state_string(cavium_atomic_t *state_ptr)
{
	s32 istate = (s32)cavium_atomic_read(state_ptr);
...
void octeon_free_device_mem(struct octeon_device *oct)
{
	...
	for (i = 0; i < MAX_OCTEON_OUTPUT_QUEUES(oct); i++) {
		if ((oct->io_qmask.oq & BIT_ULL(i)) &&
		    (oct->droq[i]))
			cavium_vfree(oct->droq[i]);
	}
	for (i = 0; i < MAX_OCTEON_INSTR_QUEUES(oct); i++) {
		if ((oct->io_qmask.iq & BIT_ULL(i)) &&
		    (oct->instr_queue[i]))
			cavium_vfree(oct->instr_queue[i]);
	}
					     ...
					     u32 priv_size)
{
	...
	cavium_spin_lock(&octeon_devices_lock);
	...
	cavium_spin_unlock(&octeon_devices_lock);
	if (!oct)
		return NULL;

	cavium_spin_lock_init(&oct->pci_win_lock);
	cavium_spin_lock_init(&oct->mem_access_lock);

	oct->octeon_id = oct_idx;
	cavium_snprintf(oct->device_name, sizeof(oct->device_name),
			   ...
			   int bus, int dev, int func, int is_pf)
{
	...
	for (idx = (int)oct->octeon_id - 1; idx >= 0; idx--) {
		if (!octeon_device[idx]) {
			lio_dev_err(oct,
				    ...
				    __CVM_FUNCTION__);
			cavium_spin_unlock(&octeon_devices_lock);
			cavium_atomic_inc(oct->adapter_refcount);
	...
	cavium_atomic_inc(oct->adapter_refcount);
	refcount = cavium_atomic_read(oct->adapter_refcount);
...
int octeon_deregister_device(struct octeon_device *oct)
{
	...
	cavium_atomic_dec(oct->adapter_refcount);
	refcount = cavium_atomic_read(oct->adapter_refcount);
...
octeon_allocate_ioq_vector(struct octeon_device  *oct, u32 num_ioqs)
{
	...
	oct->ioq_vector = cavium_vmalloc(size);
	if (!oct->ioq_vector)
		return -1;

	cavium_memset(oct->ioq_vector, 0, size);
	for (i = 0; i < num_ioqs; i++) {
		...
		cavium_cpumask_set_cpu(cpu_num, &ioq_vector->affinity_mask);
...
int octeon_setup_instr_queues(struct octeon_device *oct)
{
	...
	oct->instr_queue[0] = cavium_vzalloc_node(sizeof(*oct->instr_queue[0]),
						  numa_node);
	if (!oct->instr_queue[0])
		oct->instr_queue[0] =
			cavium_vzalloc(sizeof(struct octeon_instr_queue));
	if (!oct->instr_queue[0])
		return 1;
	...
	if (octeon_init_instr_queue(oct, txpciq, num_descs)) {
		/* prevent memory leak */
		cavium_vfree(oct->instr_queue[0]);
...
int octeon_setup_output_queues(struct octeon_device *oct)
{
	...
	u32 desc_size = 0;
	u32 oq_no = 0;
	int numa_node = cavium_droq_to_node(oct, oq_no);
	...
	oct->num_oqs = 0;
	oct->droq[0] = cavium_vmalloc_node(sizeof(*oct->droq[0]), numa_node);
	if (!oct->droq[0])
		oct->droq[0] = cavium_vmalloc(sizeof(*oct->droq[0]));
	if (!oct->droq[0])
		return 1;
	cavium_memset(oct->droq[0], 0, sizeof(*oct->droq[0]));

	if (octeon_init_droq(oct, oq_no, num_descs, desc_size, NULL)) {
		cavium_vfree(oct->droq[oq_no]);
		oct->droq[oq_no] = NULL;
		return 1;
	}
	oct->num_oqs++;
...
{
	u32 i;
	struct cavium_list_head freelist, *temp, *tmp2;
	...
	for (i = 0; i < DISPATCH_LIST_SIZE; i++) {
		struct cavium_list_head *dispatch;

		dispatch = &oct->dispatch.dlist[i].list;
		while (CAVIUM_LIST_NEXT(dispatch) != dispatch) {
			temp = CAVIUM_LIST_NEXT(dispatch);
			    ...
			    octeon_dispatch_fn_t fn, void *fn_arg)
{
	...
	if (!pfn) {
		...
		dispatch = (struct octeon_dispatch *)
			   cavium_vmalloc(sizeof(struct octeon_dispatch));
		if (!dispatch) {
			lio_dev_err(oct,
			      ...
			      u16 subcode)
{
	int retval = 0;
	u32 idx;
	struct cavium_list_head *dispatch, *dfree = NULL, *tmp2;
	u16 combined_opcode = OPCODE_SUBCODE(opcode, subcode);
	...
	if (oct->dispatch.dlist[idx].opcode == combined_opcode) {
		...
		if (CAVIUM_LIST_NEXT(dispatch) != dispatch) {
			dispatch = CAVIUM_LIST_NEXT(dispatch);
		...
		retval = 1;
		CAVIUM_LIST_FOR_EACH_SAFE(dispatch, tmp2,
...
#if (OCTEON_OQ_INFOPTR_MODE)
	cavium_memcpy(cs, get_rbd(recv_pkt->buffer_ptr[0]), sizeof(*cs));
#else
	cavium_memcpy(cs, get_rbd(recv_pkt->buffer_ptr[0]) + OCT_DROQ_INFO_SIZE,
		      sizeof(*cs));
#endif
	cavium_strncpy(oct->boardinfo.name, cs->boardname, OCT_BOARD_NAME);
	cavium_strncpy(oct->boardinfo.serial_number, cs->board_serial_number,
		OCT_SERIAL_LEN);
...
{
	if (oct && (q_no < (u32)MAX_OCTEON_INSTR_QUEUES(oct)) &&
...
{
	if (oct && (q_no < (u32)MAX_OCTEON_OUTPUT_QUEUES(oct)) &&
...
u64 lio_pci_readq(struct octeon_device *oct, u64 addr)
{
	...
	cavium_volatile_u32 val32, addrhi;

	cavium_spin_lock_irqsave(&oct->pci_win_lock, flags);
	...
	cavium_spin_unlock_irqrestore(&oct->pci_win_lock, flags);
...
void lio_enable_irq(struct octeon_droq *droq, struct octeon_instr_queue *iq)
{
	...
	if (droq) {
		pkts_pend = (u32)cavium_atomic_read(&droq->pkts_pending);
		OCTEON_WRITE32(droq->pkts_sent_reg, droq->pkt_count - pkts_pend);
	...
	if (oct && (OCTEON_CN23XX_PF(oct) || OCTEON_CN23XX_VF(oct))) {
		if (droq)
			OCTEON_WRITE64(droq->pkts_sent_reg, CN23XX_INTR_RESEND);
			/* we race with firmrware here. */
			/* read and write the IN_DONE_CNTS */
		else if (iq) {
			instr_cnt =  OCTEON_READ64(iq->inst_cnt_reg);
