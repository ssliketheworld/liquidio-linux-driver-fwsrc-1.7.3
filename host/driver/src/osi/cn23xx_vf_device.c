...
static int cn23xx_vf_soft_reset(
			struct octeon_device *oct UNUSED)
{
	return 0;
}

/* TODO: Check if these function not for VF */
void cn23xx_enable_vf_error_reporting(struct octeon_device *oct)
{
	u32 regval;

	OCTEON_READ_PCI_CONFIG(oct, CN23XX_CONFIG_PCIE_DEVCTL, &regval);
	if (regval & 0x000c0000) {
...
static u32 cn23xx_coprocessor_clock(
			struct octeon_device *oct UNUSED)
{
...
static void cn23xx_vf_mbox_thread(struct cavium_work *work)
{
...
{
	cavium_cancel_delayed_work_sync(&oct->mbox[0]->mbox_poll_wk.work);
	cavium_vfree(oct->mbox[0]);
	return 0;
}
				    ...
				    void *arg)
{
	...
	cavium_memcpy((uint8_t *)&oct->pfvf_hsword, cmd->msg.s.params,
		      CN23XX_MAILBOX_MSGPARAM_SIZE);
	if (cmd->recv_len > 1)  {
		major = (u32)((struct lio_version *)(cmd->data))->major;
		major = major << 16;
	}

	cavium_atomic_set((cavium_atomic_t *)arg, major | 1);
}
...
int cn23xx_octeon_pfvf_handshake(struct octeon_device *oct)
{
	...
	lio_dev_dbg(oct, "requesting info from pf\n");
	...
	cavium_atomic_set(&status, 0);

	do {
		cavium_sleep_timeout(1);
	} while ((!cavium_atomic_read(&status)) && (count++ < 100000));

	ret = cavium_atomic_read(&status);
	if (!ret) {
		lio_dev_err(oct, "octeon_pfvf_handshake timeout\n");
		return 1;
	}
	...
	lio_dev_dbg(oct, "got data from pf pkind is %d\n",
...
int cn23xx_setup_octeon_vf_device(struct octeon_device *oct)
{
	...
	if (!cn23xx->conf) {
		lio_dev_err(oct, "%s No Config found for CN23XX\n",
			    __CVM_FUNCTION__);
	...
	oct->fn_list.soft_reset = cn23xx_vf_soft_reset;
