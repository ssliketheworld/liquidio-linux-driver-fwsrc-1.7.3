					...
					const char *name, u32 flags)
{
	...
	if (named_addr) {
		...
		cavium_strncpy(desc->name, name, sizeof(desc->name));
					     ...
					     u32 flags)
{
	...
	if (!__cvmx_bootmem_check_version(oct, 3)) {
		...
		for (i = 0; i < num_blocks; i++) {
			...
			if (name && named_size) {
				char *name_tmp =
					cavium_kmalloc(name_length + 1, __CAVIUM_MEM_GENERAL);
				if (!name_tmp)
					break;
				...
				CVMX_BOOTMEM_NAMED_GET_NAME(oct, named_addr,
							    name_tmp,
							    name_length);
				if (!cavium_strncmp(name, name_tmp, name_length)) {
					result = named_addr;
					cavium_kfree(name_tmp);
					break;
				}
				cavium_kfree(name_tmp);
...
int octeon_console_send_cmd(struct octeon_device *oct, char *cmd_str,
			    u32 wait_hundredths)
{
	u32 len = (u32)cavium_strlen(cmd_str);
			       ...
			       u32 wait_time_hundredths)
{
	...
	/* adjust wait_time_hundredths if CAVIUM_TICKS_PER_SEC
	 * is not a multiple of 100
	 */
	r = CAVIUM_TICKS_PER_SEC % 100;
	if (r) {
		n = wait_time_hundredths * r;
		d = CAVIUM_TICKS_PER_SEC;
		q = n / d;
	...
	while (wait_time_hundredths > 0 &&
		...
		if (--wait_time_hundredths == 0)
			return -1;
		cavium_sleep_timeout(CAVIUM_TICKS_PER_SEC / 100);
	}
	return 0;
}
				...
				s32 bytes_read)
{
	...
	if (line != &console_buffer[bytes_read]) {
		...
		len = cavium_strlen(console->leftover);
		cavium_strncpy(&console->leftover[len], line,
...
static void check_console(struct cavium_work *work)
{
	...
	cavium_schedule_delayed_work(&wk->work, delay);
}
...
int octeon_init_consoles(struct octeon_device *oct)
{
	...
	/* Use BAR1_INDEX15 to create a static mapping to a region of Octeon's
	 * DRAM that contains the PCI console named block.
	 */
	oct->console_nb_info.bar1_index = 15;
	oct->fn_list.bar1_idx_setup(oct, addr,
			oct->console_nb_info.bar1_index, 1);
...
void octeon_remove_consoles(struct octeon_device *oct)
{
	...
	for (i = 0; i < oct->num_consoles; i++) {
		...
		cavium_cancel_delayed_work_sync(&oct->console_poll_work[i].work);
		console->addr = 0;
			       ...
			       char *buffer, u32 buf_size)
{
	...
	bytes_to_read = cavium_min_t(s32, bytes_to_read, buf_size);
...
static void octeon_get_uboot_version(struct octeon_device *oct)
{
	...
	ret = octeon_console_send_cmd(oct, "setenv stdout pci", 50);

	console = &oct->console[console_num];
	tries = 0;
	total_read = 0;

	ret = octeon_console_send_cmd(oct, "version", 1);
	...
	ret = octeon_console_send_cmd(oct, "setenv stdout serial", 50);
			     ...
			     size_t size)
{
	...
	if (cavium_be32_to_cpu(h->magic) != LIO_NIC_MAGIC) {
		lio_dev_err(oct, "Unrecognized firmware file.\n");
		return -EINVAL;
	}

	crc32_result =
		cavium_crc32(~0U, data,
		      sizeof(struct octeon_firmware_file_header) -
		      sizeof(u32)) ^ ~0U;
	if (crc32_result != cavium_be32_to_cpu(h->crc32)) {
		lio_dev_err(oct, "Firmware CRC mismatch (0x%08x != 0x%08x).\n",
			    crc32_result, cavium_be32_to_cpu(h->crc32));
		return -EINVAL;
	}

	if (cavium_strncmp(LIQUIDIO_PACKAGE, h->version,
		    cavium_strlen(LIQUIDIO_PACKAGE)) != 0) {
		lio_dev_err(oct, "Unmatched firmware package type. Expected %s, got %s.\n",
			    LIQUIDIO_PACKAGE, h->version);
		return -EINVAL;
	}

	if (cavium_memcmp(LIQUIDIO_BASE_VERSION,
		   h->version + cavium_strlen(LIQUIDIO_PACKAGE),
	...
	if (cavium_be32_to_cpu(h->num_images) > LIO_MAX_IMAGES) {
		lio_dev_err(oct, "Too many images in firmware file (%d).\n",
			    cavium_be32_to_cpu(h->num_images));
		return -EINVAL;
	}

	lio_dev_info(oct, "Firmware version: %s\n", h->version);
	cavium_snprintf(oct->fw_info.liquidio_firmware_version, 32,
			"LIQUIDIO: %s", h->version);

	data += sizeof(struct octeon_firmware_file_header);

	lio_dev_info(oct, "LIQUIDIO: Firmware Download: Loading %d images\n",
		     cavium_be32_to_cpu(h->num_images));
	/* load all images */
	for (i = 0; i < cavium_be32_to_cpu(h->num_images); i++) {
		load_addr = cavium_be64_to_cpu(h->desc[i].addr);
		image_len = cavium_be32_to_cpu(h->desc[i].len);
		...
		while (rem) {
			...
			/* download the image */
			octeon_pci_write_core_mem(oct, load_addr, (u8 *)data,
						  (u32)size);
	...
	max_bootcmd_len = LIO_MAX_BOOTCMD_LEN + MAX_BOOTTIME_SIZE;
	bootcmd = cavium_vmalloc(max_bootcmd_len);
	if (!bootcmd) {
		lio_dev_err(oct, "Failed to allocate memory for bootcmd\n");
		return -ENOMEM;
	}
	cavium_snprintf(bootcmd, max_bootcmd_len,
			"%s time_sec=%lld time_nsec=%ld",
			h->bootcmd, (s64)ts.tv_sec, ts.tv_nsec);
	/* Invoke the bootcmd */
	lio_dev_info(oct, "Writing boot command: %s\n", bootcmd);
	ret = octeon_console_send_cmd(oct, bootcmd, 50);

	if (ret)
		lio_dev_info(oct, "Boot command send failed\n");

	cavium_vfree(bootcmd);
	return ret;
}
