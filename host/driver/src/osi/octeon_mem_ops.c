...
static void
octeon_pci_fastwrite(struct octeon_device *oct, u8 cavium_iomem *mapped_addr,
		     u8 *hostbuf, u32 len)
{
	while ((len) && ((size_t)mapped_addr) & 7) {
	...
	while (len >= 8) {
		OCTEON_WRITE64(mapped_addr, *((u64 *)hostbuf));
		mapped_addr += 8;
		hostbuf += 8;
		len -= 8;
	}
...
static void
octeon_pci_fastread(struct octeon_device *oct, u8 cavium_iomem *mapped_addr,
		    u8 *hostbuf, u32 len)
{
	while ((len) && ((size_t)mapped_addr) & 7) {
	...
	while (len >= 8) {
		*((u64 *)hostbuf) = OCTEON_READ64(mapped_addr);
		mapped_addr += 8;
		hostbuf += 8;
		len -= 8;
	}
			 ...
			 u8 *hostbuf, u32 len, u32 op)
{
	...
	if (static_mapping_base && static_mapping_base ==
		...
		mapped_addr = oct->mmio[1].hw_addr + (bar1_index << 22) +
			(addr & 0x3fffff);
	...
	cavium_spin_lock_irqsave(&oct->mem_access_lock, flags);
	...
	cavium_spin_unlock_irqrestore(&oct->mem_access_lock, flags);
}
