			       ...
			       u32		       rdatasize)
{
	...
	cavium_memcpy(&sc->cmd, cmd, sizeof(union octeon_instr_64B));
			  ...
			  struct octnic_ctrl_pkt *nctrl)
{
	...
	cavium_init_completion(&sc->complete);
			 ...
			 struct octnic_ctrl_pkt *nctrl)
{
	...
	if (retval == IQ_SEND_FAILED) {
		...
		lio_dev_err(oct, "%s pf_num:%d soft command:%d send failed status: %x\n",
			    __CVM_FUNCTION__, oct->pf_num, nctrl->ncmd.s.cmd, retval);
		cavium_spin_unlock_softirqrestore(&oct->cmd_resp_wqlock);
		return -1;
	}
	...
	retval = nctrl->sc_status = sc->sc_status;
	if (nctrl->cb_fn)
		nctrl->cb_fn(nctrl);
