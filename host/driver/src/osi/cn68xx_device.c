...
static inline void lio_cn68xx_vendor_message_fix(struct octeon_device *oct)
{
	...
	OCTEON_READ_PCI_CONFIG(oct, CN6XXX_PCIE_FLTMSK, &val);
	val |= 0x3;
	OCTEON_WRITE_PCI_CONFIG(oct, CN6XXX_PCIE_FLTMSK, val);
}
...
int lio_setup_cn68xx_octeon_device(struct octeon_device *oct)
{
	...
	cavium_spin_lock_init(&cn68xx->lock_for_droq_int_enb_reg);
