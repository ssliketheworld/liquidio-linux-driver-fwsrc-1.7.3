...
module_param(num_queues, uint, 0000);
MODULE_PARM_DESC(num_queues, "number of queues to be configured on this VF interface. Will be limited by the number allowed by PF");
...
liquidio_pcie_mmio_enabled(struct pci_dev *pdev UNUSED)
{
 ...
 */
static pci_ers_result_t
liquidio_pcie_slot_reset(struct pci_dev *pdev UNUSED)
{
	/* We should never hit this since we never ask for a reset for a Fatal
	 * Error. We always return DISCONNECT in io_error above.
	 * But play safe and return RECOVERED for now.
 ...
 */
static int liquidio_vf_suspend(struct pci_dev *pdev UNUSED,
 ...
 * @param pdev Pointer to PCI device
 */
static int liquidio_vf_resume(struct pci_dev *pdev UNUSED)
{
	return 0;
}
#endif
...
static struct pci_error_handlers liquidio_vf_err_handler = {
#else
...
#if  LINUX_VERSION_CODE < KERNEL_VERSION(3, 8, 0)
	.remove		= __devexit_p(liquidio_vf_remove),
#else
	.remove		= liquidio_vf_remove,
#endif
	.err_handler	= &liquidio_vf_err_handler,    /* For AER */
#ifdef CONFIG_PM
	.suspend	= liquidio_vf_suspend,
	.resume		= liquidio_vf_resume,
#endif
};
...
#endif
{
	...
	octeon_register_device(oct_dev, bus, dev, func, false);
...
#define PCI_DMA_64BIT 0xffffffffffffffffULL
	...
	if (pci_set_consistent_dma_mask(oct->pci_dev, PCI_DMA_64BIT)) {
		lio_dev_err(oct,
			    "pci_set_coherent_mask(64bit) failed\n");
		return 1;
	}
#else
...
static void liquidio_set_uc_list(struct net_device *netdev)
{
	...
	if (netdev_uc_count(netdev) > MAX_NCTRL_UDD) {
		lio_dev_err(oct, "too many MAC addresses in netdev uc list\n");
		return;
	}
	...
	netdev_for_each_uc_addr(ha, netdev) {
		...
		memcpy(((u8 *)mac) + 2, ha->addr, ETH_ALEN);
#else
...
static int liquidio_set_mac(struct net_device *netdev, void *p)
{
	...
	if (nctrl.sc_status == FIRMWARE_STATUS_CODE(OCTEON_REQUEST_NO_PERMISSION)) {
		lio_dev_err(oct, "MAC Address change failed: no permission\n");
		return -EPERM;
	}
...
static struct net_device_stats *liquidio_get_stats(struct net_device *netdev)
{
	...
	/* TODO: fill collisions, rx_fifo_errors, rx_missed_errors
	 * and rx_over_errors */
	...
	/* TODO: fill tx_fifo_errors, tx_heartbeat_errors
	 * and tx_window_errors */
...
static int liquidio_xmit(struct sk_buff *skb, struct net_device *netdev)
{
	...
	if (!(cavium_atomic_read(&lio->ifstate) & LIO_IFSTATE_RUNNING) ||
		...
		lio_info(lio, tx_err, "Transmit failed skb->len : %d  link_status : %d\n",
...
/* HW insert VLAN tag */
	...
	if (vlan_tx_tag_present(skb)) {
		irh->priority = vlan_tx_tag_get(skb) >> VLAN_PRIO_SHIFT;
		irh->vlan = vlan_tx_tag_get(skb) & VLAN_VID_MASK;
	}
#endif
	...
	if (status == IQ_SEND_STOP) {
		lio_dev_err(oct, "Rcvd IQ_SEND_STOP signal; stopping IQ-%d\n",
			iq_no);
...
#endif
{
	...
	if (lio->linfo.vlan_is_admin_assigned) {
		ret = -EPERM;
	} else {
		memset(&nctrl, 0, sizeof(struct octnic_ctrl_pkt));
...
static int setup_nic_devices(struct octeon_device *octeon_dev)
{
	...
	for (i = 0; i < octeon_dev->ifcount; i++) {
		...
		*((u64 *)vdata) = 0;
		...
		lio->linfo.vlan_is_admin_assigned =
			resp->cfg_info.linfo.vlan_is_admin_assigned;
...
#endif
		/* Set the mtu from if_cfg response */
		netdev->mtu = resp->cfg_info.linfo.link.s.mtu;
...
/* Copy MAC Address to OS network device structure */
		...
		liquidio_set_feature(netdev, OCTNET_CMD_PKT_STEERING_CTL,
...
static int octeon_device_init(struct octeon_device *oct)
{
	...
	cavium_atomic_set(&oct->status, OCT_DEV_BEGIN_STATE);
	...
	cavium_atomic_set(&oct->status, OCT_DEV_PCI_ENABLE_DONE);
	...
	if (num_queues != 0) {
		if (num_queues < 1 || num_queues > CN23XX_MAX_RINGS_PER_VF) {
			num_queues = CN23XX_MAX_RINGS_PER_VF;
			lio_dev_info(oct,
				     "num_queues is out of range (1...%d). changing num_queues to %d\n",
				     CN23XX_MAX_RINGS_PER_VF,
				     CN23XX_MAX_RINGS_PER_VF);
		}
		oct->sriov_info.rings_per_vf = num_queues;
	}
	...
	cavium_atomic_set(&oct->status, OCT_DEV_DISPATCH_INIT_DONE);
	...
	if (oct->fn_list.setup_device_regs(oct)) {
		lio_dev_err(oct, "device registers configuration failed\n");
		return 1;
	}

	/* Initialize soft command buffer pool */
	if (octeon_setup_sc_buffer_pool(oct)) {
		lio_dev_err(oct, "sc buffer pool allocation failed\n");
		return 1;
	}
	cavium_atomic_set(&oct->status, OCT_DEV_SC_BUFF_POOL_INIT_DONE);

	/* Setup the data structures that manage this Octeon's Input queues. */
	if (octeon_setup_instr_queues(oct)) {
		lio_dev_err(oct, "instruction queue initialization failed\n");
		return 1;
	}
	...
	if (octeon_setup_response_list(oct)) {
		lio_dev_err(oct, "Response list allocation failed\n");
		return 1;
	}
	cavium_atomic_set(&oct->status, OCT_DEV_RESP_LIST_INIT_DONE);

	if (octeon_setup_output_queues(oct)) {
		lio_dev_err(oct, "Output queue initialization failed\n");
		return 1;
	}
	...
	cavium_atomic_set(&oct->status, OCT_DEV_MBOX_SETUP_DONE);

	if (octeon_allocate_ioq_vector(oct, oct->sriov_info.rings_per_vf)) {
		lio_dev_err(oct, "ioq vector allocation failed\n");
		return 1;
	}
	cavium_atomic_set(&oct->status, OCT_DEV_MSIX_ALLOC_VECTOR_DONE);

	tasklet_init(&oct_priv->droq_tasklet, octeon_droq_bh,
		     (unsigned long)oct);

	lio_dev_info(oct, "OCTEON_CN23XX VF Version: %s, %d ioqs\n",
		     LIQUIDIO_VERSION, oct->sriov_info.rings_per_vf);

	/* Setup the interrupt handler and record the INT SUM
	 * register address
	 */
	...
	cavium_atomic_set(&oct->status, OCT_DEV_INTR_SET_DONE);
	...
	if (oct->fn_list.enable_io_queues(oct)) {
		lio_dev_err(oct, "enabling io queues failed\n");
		return 1;
	}

	cavium_atomic_set(&oct->status, OCT_DEV_IO_QUEUES_DONE);
