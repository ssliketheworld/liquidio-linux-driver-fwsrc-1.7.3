...
#define OCT_ETHTOOL_REGDUMP_LEN_23XX  49248
...
static const char oct_vf_priv_flags_strings[][ETH_GSTRING_LEN] = {
	"pkt_steering",
};
...
#define ETHTOOL_LINK_MODE_25000baseCR_Full_BIT  (1 << 31)
#define ETHTOOL_LINK_MODE_25000baseKR_Full_BIT  (1 << 32)
#define ETHTOOL_LINK_MODE_25000baseSR_Full_BIT  (1 << 33)
				  ...
				  struct ethtool_link_ksettings *ecmd)
{
	...
	case LIO_PHY_PORT_FIBRE:
		if (linfo->link.s.if_mode == INTERFACE_MODE_XAUI ||
			...
			lio_dev_dbg(oct, "%s: ecmd->base.transceiver is XCVR_EXTERNAL\n",__func__);
		} else {
			lio_dev_err(oct, "Unknown link interface reported %d\n",
		...
		if (oct->subsystem_id == OCTEON_CN2350_25GB_SUBSYS_ID ||
			...
			if (OCTEON_CN23XX_PF(oct)) {
				...
				if (oct->no_speed_setting == 0)  {
					ethtool_link_ksettings_add_link_mode
						(ecmd, supported, 10000baseKR_Full);
				}
				...
				if (oct->no_speed_setting == 0) {
					...
					ethtool_link_ksettings_add_link_mode
						(ecmd, advertising, 10000baseKR_Full);
                        	}
                        	if (oct->speed_setting == 25) {
					ethtool_link_ksettings_add_link_mode
						(ecmd, advertising, 25000baseSR_Full);
					ethtool_link_ksettings_add_link_mode
						(ecmd, advertising, 25000baseKR_Full);
					ethtool_link_ksettings_add_link_mode
						(ecmd, advertising, 25000baseCR_Full);
                        	}

				if (oct->speed_setting != oct->speed_boot) {
					lio_dev_info(oct,"Reload the driver to change speed from %dG to %dG\n", 
						oct->speed_boot, oct->speed_setting);
				}

#if LINUX_VERSION_CODE >= KERNEL_VERSION(4, 14, 0)
				if (oct->no_speed_setting == 0) {
					ethtool_link_ksettings_add_link_mode(ecmd, supported, FEC_RS);
					ethtool_link_ksettings_add_link_mode(ecmd, supported, FEC_NONE);
						/*FEC_OFF*/
					if (oct->props[lio->ifidx].fec == 1) {
						/* ETHTOOL_FEC_RS */
						ethtool_link_ksettings_add_link_mode(ecmd, advertising, FEC_RS);

					} else {
						/* ETHTOOL_FEC_OFF */
						ethtool_link_ksettings_add_link_mode(ecmd, advertising, FEC_NONE);
					}
				}
#endif
			} else { /* VF */
...
static int lio_get_settings(struct net_device *netdev, struct ethtool_cmd *ecmd)
{
	...
	case LIO_PHY_PORT_FIBRE:
		...
		/* TODO: ethtool doesn't have a parameter for 25G speed */
	...
	default:
		lio_dev_err(oct, "Invalid PHY type %d\n",
			    linfo->link.s.phy_type);
		ecmd->port = PORT_NONE;
		break;
	}
				  ...
				  const struct ethtool_link_ksettings *ecmd)
{
	...
	if (oct->no_speed_setting) {
		lio_dev_err(oct, "%s: Changing speed is not supported\n", __func__);
		return -EOPNOTSUPP;
	}

        if (((ecmd->base.duplex != DUPLEX_UNKNOWN) && (ecmd->base.duplex != linfo->link.s.duplex) ) ||
            (ecmd->base.autoneg != AUTONEG_DISABLE) ||
            ((ecmd->base.speed != 10000) && (ecmd->base.speed != 25000) && (ecmd->base.speed != SPEED_UNKNOWN))) {
                lio_dev_err(oct, "set_settings: Unsupported port change.\n");
                return -EOPNOTSUPP;
        }

	if ((oct->speed_boot == speed/1000) && (oct->speed_boot == oct->speed_setting)) {
                return 0;
        }

        liquidio_set_speed(lio, speed/1000);

	/* speed settings apply to the entire adapter so the module must be reloaded */
	lio_dev_info(oct, "speed is set to %d\n", speed);
	if (oct->speed_boot != oct->speed_setting)
		lio_dev_info(oct, "Port speed will be %d next time module is reload\n", speed);
...
static int lio_set_settings(struct net_device *netdev, struct ethtool_cmd *ecmd)
{
	...
	if (oct->subsystem_id == OCTEON_CN2350_25GB_SUBSYS_ID ||
		...
		lio_dev_err(oct, "set_settings: no support\n");	
		return -EOPNOTSUPP;
	}
	...
	if (((ecmd->duplex != DUPLEX_UNKNOWN) && (ecmd->duplex != linfo->link.s.duplex) ) ||
	    (ecmd->autoneg != AUTONEG_DISABLE) ||
	    ((ecmd->speed != 10000) && (ecmd->speed != 25000) && (ecmd->speed != SPEED_UNKNOWN))) {
		lio_dev_err(oct, "set_settings: unsupport speed chang.\n");
		return -EOPNOTSUPP;
	}

	if ((oct->speed_boot == speed/1000) && (oct->speed_boot == oct->speed_setting)) {
		lio_dev_dbg(oct, "%s: no op: oct->speed_boot=%d, speed=%d\n", __FUNCTION__, oct->speed_boot, speed);
		return 0;
	}
	...
	lio_dev_info(oct, "speed is set to %d\n", speed);
	if (oct->speed_boot != oct->speed_setting)
		lio_dev_info(oct, "speed will be %d next time reload Liquidio\n", speed);
...
lio_get_drvinfo(struct net_device *netdev, struct ethtool_drvinfo *drvinfo)
{
	...
	if (oct->chip_id == OCTEON_CN23XX_PF_VID)
		drvinfo->regdump_len = OCT_ETHTOOL_REGDUMP_LEN_23XX;
	else if (oct->chip_id == OCTEON_CN23XX_VF_VID)
		drvinfo->regdump_len = OCT_ETHTOOL_REGDUMP_LEN_23XX_VF;
	else
...
#if (LINUX_VERSION_CODE > KERNEL_VERSION(3, 0, 0)) ||  (RHEL_RELEASE_CODE > RHEL_RELEASE_VERSION(6, 3))
static void
...
lio_irq_reallocate_irqs(struct octeon_device *oct, uint32_t num_ioqs)
{
	...
	if (octeon_allocate_ioq_vector(oct, num_ioqs)) {
		lio_dev_err(oct, "OCTEON: ioq vector allocation failed\n");
		return -1;
	}

	if (octeon_setup_interrupt(oct, num_ioqs)) {
		lio_dev_info(oct, "Setup interrupt failed\n");
		return -1;
	}
			 ...
			 struct ethtool_channels *channel)
{
	...
	if (strcmp(oct->fw_info.liquidio_firmware_version, "1.6.1") < 0) {
		lio_dev_err(oct, "Minimum firmware version required is 1.6.1\n");
		return -EINVAL;
	}
...
#define PER_RW_SIZE 1536
static int
lio_get_dump_flag(struct net_device *netdev, struct ethtool_dump *dump)
{
	struct lio *lio = GET_LIO(netdev);
	struct octeon_device *oct_dev = lio->oct_dev;
	struct octeon_soft_command *sc = NULL;
	struct flash_rw_resp *flash_resp;
	int retval;

	sc = (struct octeon_soft_command *)
		octeon_alloc_soft_command(oct_dev,
				0, sizeof(struct flash_rw_resp), 0);
	if (!sc) {
		lio_dev_err(oct_dev, "%s: Failed to allocate soft command\n",
				__func__);
		return -ENOMEM;
	}

	flash_resp = (struct flash_rw_resp *)sc->virtrptr;
	flash_resp->op_status = -1;

	sc->iq_no = lio->linfo.txpciq[0].s.q_no;
	octeon_prepare_soft_command(oct_dev, sc, OPCODE_NIC,
			OPCODE_NIC_GET_DUMP_FLAG, 0, 0, 0);
	...
	if (retval == IQ_SEND_FAILED) {
		lio_dev_err(oct_dev,
			    "dump flag request send failed status: %x\n",
			    retval);
		retval = -EBUSY;
		octeon_free_soft_command(oct_dev, sc);
		goto get_dump_flag_fail;
	}

	retval = cavium_sleep_cond_timeout(oct_dev, sc, 0);
	if (retval)
		goto get_dump_flag_fail;

	retval = cavium_be32_to_cpu(flash_resp->op_status);
	if (!retval) {
		dump->len = cavium_cpu_to_be32(flash_resp->dump_len);
		dump->flag = cavium_cpu_to_be32(flash_resp->dump_flag);
	} else {
		lio_dev_err(oct_dev,
			    "flash length request failed status: %x\n",
			    retval);
		retval = -EOPNOTSUPP;
	}
...
static int
lio_get_dump_data(struct net_device *netdev, struct ethtool_dump *dump,
		void *buffer)
{
	struct lio *lio = GET_LIO(netdev);
	struct octeon_device *oct_dev = lio->oct_dev;
	struct flash_rw_req *flash_req;
	struct flash_rw_resp *flash_resp;
	struct octeon_soft_command *sc;
	struct ethtool_dump lio_dump;
	int to_read, offset, retval, rsize;
	uint8_t *dump_ptr = (uint8_t *)buffer;

	if (lio_get_dump_flag(netdev, &lio_dump) < 0)
		return -EOPNOTSUPP;

	if (dump->len > lio_dump.len)
		return -EINVAL;
	...
	while (to_read) {
		...
		rsize = (to_read > PER_RW_SIZE) ? PER_RW_SIZE:to_read;

		sc = (struct octeon_soft_command *)
		octeon_alloc_soft_command(oct_dev,
				sizeof(struct flash_rw_req),
				sizeof(struct flash_rw_resp) + rsize + 8, 0);
		if (!sc) {
			lio_dev_err(oct_dev, "%s: Failed to allocate soft command\n",
					__func__);
			return -ENOMEM;
		...
		flash_resp = (struct flash_rw_resp *)sc->virtrptr;
		flash_req  = (struct flash_rw_req *)sc->virtdptr;

		flash_req->off = cavium_cpu_to_be32(offset);
		flash_req->len = cavium_cpu_to_be32(rsize);
		flash_resp->op_status = -1;

		sc->iq_no = lio->linfo.txpciq[0].s.q_no;
		octeon_prepare_soft_command(oct_dev, sc, OPCODE_NIC,
				OPCODE_NIC_GET_DUMP, 0, 0, 0);
		...
		if (retval == IQ_SEND_FAILED) {
			lio_dev_err(oct_dev,
				    "fw_dump read request send failed status: %x\n",
				    retval);
			retval = -EBUSY;
			octeon_free_soft_command(oct_dev, sc);
			goto get_dump_fail;
		...
		if (!retval) {
			to_read -= rsize;
			memcpy(dump_ptr + offset, (u8 *)(flash_resp + 1),
					rsize);
			offset += rsize;
			cavium_set_bit(CALLER_DONE_BIT, &sc->done);
		} else {
			lio_dev_err(oct_dev,
				    "fw_dump read request failed status: %x\n",
				    retval);
			retval = -EOPNOTSUPP;
			cavium_set_bit(CALLER_DONE_BIT, &sc->done);
			goto get_dump_fail;
...
static int
lio_set_dump(struct net_device *netdev, struct ethtool_dump *val)
{
	struct lio *lio = GET_LIO(netdev);
	struct octeon_device *oct_dev = lio->oct_dev;
	struct flash_rw_req  *flash_req;
	...
	flash_req->flag = cavium_cpu_to_be32(val->flag);
	//flash_req->len = cavium_cpu_to_be32(val->len);

	flash_resp->op_status = -1;

	sc->iq_no = lio->linfo.txpciq[0].s.q_no;
	octeon_prepare_soft_command(oct_dev, sc, OPCODE_NIC,
			OPCODE_NIC_SET_DUMP, 0, 0, 0);
	...
	if (retval == IQ_SEND_FAILED) {
		lio_dev_err(oct_dev,
			    "fw_dump flag set request send failed status: %x\n",
			    retval);
		retval = -EBUSY;
		octeon_free_soft_command(oct_dev, sc);
		goto set_dump_flag_fail;
	}
	...
	if (retval) {
		lio_dev_err(oct_dev,
			    "fw_dump flag set request failed status: %x\n",
			    retval);
		retval = -EOPNOTSUPP;
	}
...
octnet_mdio45_access(struct lio *lio, int op, int loc, int *value)
{
	...
	if (retval == IQ_SEND_FAILED) {
		 ...
		 * response arrived
		 */
		if ((retval = cavium_sleep_cond_timeout(oct_dev, sc, 0))) {
			return (retval);
		} else {
...
int lio_23xx_reconfigure_queue_count(struct lio *lio)
{
	...
	sc = (struct octeon_soft_command *)
		octeon_alloc_soft_command(oct, data_size,
					  resp_size, 0);
	if (!sc) {
		lio_dev_err(oct, "%s: Failed to allocate soft command\n",
			    __func__);
		return -1;
	}
...
static int lio_reset_queues(struct net_device *netdev, uint32_t num_qs)
{
	...
	cavium_sleep_timeout(msecs_to_jiffies(100));
	...
	if (octeon_set_io_queues_off(oct)) {
		lio_dev_err(oct, "Setting io queues off failed\n");
		return -1;
	}
	...
	if (queue_count_update) {
		...
		if (OCTEON_CN23XX_PF(oct) && !oct->sriov_info.sriov_enabled) {
			if (oct->fn_list.setup_mbox(oct)) {
				lio_dev_err(oct, "Mailbox setup failed\n");
				return -1;
			}
		}
		...
		if (lio_irq_reallocate_irqs(oct, num_qs)) {
			lio_dev_err(oct, "IRQs could not be allocated\n");
			return -1;
		}

		/* Enable the input and output queues for this Octeon device */
		if (oct->fn_list.enable_io_queues(oct)) {
			lio_dev_err(oct, "Failed to enable input/output queues\n");
			return -1;
		}
	...
	if (liquidio_setup_io_queues(oct, 0, num_qs, num_qs)) {
		lio_dev_err(oct, "I/O queues creation failed\n");
		return -1;
	}

	if (queue_count_update) {
		if (lio_setup_glists(oct, lio, num_qs)) {
			lio_dev_err(oct, "Gather list allocation failed\n");
			return -1;
		}

		if (setup_rx_oom_poll_fn(netdev)) {
			lio_dev_err(oct, "lio_setup_rx_oom_poll_fn failed\n");
			return 1;
		}
...
lio_get_pauseparam(struct net_device *netdev, struct ethtool_pauseparam *pause)
{
	...
	if (atomic_read(&lio->oct_dcb.dcb_state) &&
	    (lio->oct_dcb.dcbx_info.operational.pfc_config.pfc_flags &
	    DCBX_FLAG_ENABLED)) {
		pause->autoneg = 0;
		pause->tx_pause = 0;
		pause->rx_pause = 0;
...
lio_get_ethtool_stats(struct net_device *netdev,
		      struct ethtool_stats *stats  UNUSED,
		      u64 *data)
{
	struct lio *lio = GET_LIO(netdev);
	struct octeon_device *oct_dev = lio->oct_dev;
	...
	} else if (netdev->netdev_ops->ndo_get_stats64) {
...
static void lio_get_priv_flags_strings(struct lio *lio, u8 *data)
{
	...
	case OCTEON_CN23XX_VF_VID:
		for (i = 0; i < ARRAY_SIZE(oct_vf_priv_flags_strings); i++) {
			sprintf(data, "%s", oct_vf_priv_flags_strings[i]);
			data += ETH_GSTRING_LEN;
		}
		break;
	case OCTEON_CN68XX:
...
static void lio_get_strings(struct net_device *netdev, u32 stringset, u8 *data)
{
	...
	case ETH_SS_PRIV_FLAGS:
		...
		break;
	default:
		lio_info(lio, drv, "Unknown Stringset !!\n");
		break;
	}
}
...
static int lio_get_priv_flags_ss_count(struct lio *lio)
{
	...
	case OCTEON_CN23XX_VF_VID:
		return ARRAY_SIZE(oct_vf_priv_flags_strings);
	case OCTEON_CN68XX:
	case OCTEON_CN66XX:
		return -EOPNOTSUPP;
	default:
				  ...
				  struct oct_intrmod_cfg *intr_cfg)
{
	 ...
	 * response arrived or timed-out.
	 */
	if ((retval = cavium_sleep_cond_timeout(oct_dev, sc, 0)) == 0) {
		...
		lio_dev_err(oct_dev, "iq/oq config failed, error=%d\n", retval);
		return retval;
	}
...
#if LINUX_VERSION_CODE < KERNEL_VERSION(3, 0, 0)
static u32 lio_get_tx_csum(struct net_device *netdev)
{
	return (netdev->features & NETIF_F_IP_CSUM) != 0;
}

static int lio_set_tx_csum(struct net_device *netdev, u32 data)
{
	if (data)
...
static u32 lio_get_rx_csum(struct net_device *netdev)
{
	struct lio *lio = GET_LIO(netdev);

	if (lio->dev_capability & NETIF_F_RXCSUM)
		return (netdev->features & NETIF_F_RXCSUM) != 0;
	else
		return 0;
}

static int lio_set_rx_csum(struct net_device *dev, u32 data)
{
	struct lio *lio = GET_LIO(dev);
	...
	if (lio->dev_capability & NETIF_F_RXCSUM) {
		...
		lio_info(lio, drv,
			 "Rx checksum offload capablity not supported\n");

	return 0;
}

static u32 lio_get_tso(struct net_device *netdev)
{
	struct lio *lio = GET_LIO(netdev);
	u32 features = 0;

	if ((lio->dev_capability & NETIF_F_TSO) &&
	    (lio->dev_capability & NETIF_F_TSO6))
		features = ((netdev->features & NETIF_F_TSO) &&
			    (netdev->features & NETIF_F_TSO6));

	return features;
}

static int lio_set_flags(struct net_device *dev, u32 data)
{
	struct lio *lio = GET_LIO(dev);

	if (!(lio->dev_capability & NETIF_F_LRO))
		return -EINVAL;

	if (data & ETH_FLAG_LRO) {
		if (!(dev->features & NETIF_F_LRO)) {
			dev->features |= NETIF_F_LRO;
			liquidio_set_feature(dev, OCTNET_CMD_LRO_ENABLE,
					     OCTNIC_LROIPV4 | OCTNIC_LROIPV6);
		}
	} else if (dev->features & NETIF_F_LRO) {
		dev->features &= ~NETIF_F_LRO;
		liquidio_set_feature(dev, OCTNET_CMD_LRO_DISABLE,
				     OCTNIC_LROIPV4 | OCTNIC_LROIPV6);
	}
...
static int lio_set_tso(struct net_device *netdev, u32 features)
{
	struct lio *lio = GET_LIO(netdev);
	int err = -EINVAL;
	...
	if (features) {
		if (lio->dev_capability & NETIF_F_TSO) {
			...
			lio_info(lio, drv, "TSO capablity not supported\n");
			return err;
		}

		if (lio->dev_capability & NETIF_F_TSO6) {
			netdev->features |= NETIF_F_TSO6;
		} else {
			lio_info(lio, drv, "TSO6 capablity not supported\n");
			return err;
		}
	} else {
...
#endif
#if (LINUX_VERSION_CODE >= KERNEL_VERSION(3, 5, 0)) ||  (RHEL_RELEASE_CODE > RHEL_RELEASE_VERSION(6, 3))
...
int lio_get_module_info(struct net_device *netdev, struct ethtool_modinfo *modinfo)
{
...
int lio_get_module_eeprom(struct net_device *dev, struct ethtool_eeprom *ee, u8 *data)
{
	struct lio *lio = GET_LIO(dev);
	struct octeon_device *oct_dev = lio->oct_dev;

	struct octeon_soft_command *sc;
	struct oct_mod_eeprom_resp *resp;
	...
	sc = (struct octeon_soft_command *)
		octeon_alloc_soft_command(oct_dev,
					  0,
					  sizeof(struct oct_mod_eeprom_resp),
					  0);

	if (!sc)
		return -ENOMEM;

	resp = (struct oct_mod_eeprom_resp *)sc->virtrptr;
	memset(resp, 0, sizeof(struct oct_mod_eeprom_resp));
	...
	if ((retval = cavium_sleep_cond_timeout(oct_dev, sc, 5000)))  {
		return retval;
	}

	if ((sc->sc_status != OCTEON_REQUEST_TIMEOUT) && !resp->status) {
		memcpy(data, resp->data, ee->len);
		retval = 0;
	} else {
		retval = -ENXIO;
	}
...
static int cn23xx_read_csr_reg(char *s, struct octeon_device *oct)
{
	...
	len +=
	    sprintf(s + len, "[%08x] (SLI_MAC%d_PF%d_INT_ENB): %016llx\n",
		    reg, oct->pcie_port, oct->pf_num,
		    (u64)octeon_read_csr64(oct, reg));
	...
	len +=
	    sprintf(s + len, "[%08x] (SLI_MAC%d_PF%d_INT_SUM): %016llx\n",
		    reg, oct->pcie_port, oct->pf_num,
		    (u64)octeon_read_csr64(oct, reg));
	...
	len += sprintf(
	    s + len, "[%08x] (SLI_MAC%d_PF%d_PKT_VF_INT): %016llx\n", reg,
	...
	for (i = 0; i < CN23XX_MAX_OUTPUT_QUEUES; i++) {
		...
		len +=
		    sprintf(s + len, "[%08x] (SLI_PKT%d_OUT_SIZE): %016llx\n",
	...
	for (i = 0; i < CN23XX_MAX_OUTPUT_QUEUES; i++) {
		...
		len += sprintf(
		    s + len, "[%08x] (SLI_PKT%d_SLIST_FIFO_RSIZE): %016llx\n",
		    reg, i, (u64)octeon_read_csr64(oct, reg));
	}
...
{
	struct lio *lio = GET_LIO(netdev);
	u32 ofec, nfec, ops, nps;
#ifdef TX_COMPLETION_INTR_BY_BYTES
	bool intr_by_tx_bytes = !!(flags & (0x1 << OCT_PRIV_FLAG_TX_BYTES));

	lio_set_priv_flag(lio->oct_dev, OCT_PRIV_FLAG_TX_BYTES,
			  intr_by_tx_bytes);
#endif
	ops = lio->oct_dev->priv_flags & (0x1 << OCT_PRIV_FLAG_PKT_STEERING);
	nps = flags & (0x1 << OCT_PRIV_FLAG_PKT_STEERING);
	if (ops != nps)
	    lio_set_pkt_steering(netdev,
			     !!(flags & (0x1 << OCT_PRIV_FLAG_PKT_STEERING)));

	ofec = lio->oct_dev->priv_flags & (0x1 << OCT_PRIV_FLAG_FEC);
	nfec = flags & (0x1 << OCT_PRIV_FLAG_FEC);
	if (ofec != nfec) {
		if (lio->oct_dev->speed_boot == 25) {
			lio->oct_dev->priv_flags = 
				(lio->oct_dev->priv_flags & ~(0x1 << OCT_PRIV_FLAG_FEC)) | nfec;
			nfec = nfec >> OCT_PRIV_FLAG_FEC;
			liquidio_set_fec(lio, nfec);
		} else {
			lio_dev_info(lio->oct_dev, "FEC setting is only supported at 25G mode\n");
		}
	}
...
static u32 lio_vf_get_priv_flags(struct net_device *netdev)
{
	struct lio *lio = GET_LIO(netdev);
...
static int lio_vf_set_priv_flags(struct net_device *netdev, u32 flags)
{
#ifdef TX_COMPLETION_INTR_BY_BYTES
	struct lio *lio = GET_LIO(netdev);
	bool intr_by_tx_bytes = !!(flags & (0x1 << OCT_PRIV_FLAG_TX_BYTES));
...
static const struct ethtool_ops lio_ethtool_ops = {
...
#if LINUX_VERSION_CODE >= KERNEL_VERSION(3, 0, 0)
	.get_dump_flag		= lio_get_dump_flag,
	.get_dump_data		= lio_get_dump_data,
	.set_dump		= lio_set_dump,
#endif
#if LINUX_VERSION_CODE < KERNEL_VERSION(3, 0, 0)
	.get_tx_csum		= lio_get_tx_csum,
	.set_tx_csum		= lio_set_tx_csum,
	.get_rx_csum		= lio_get_rx_csum,
	.set_rx_csum		= lio_set_rx_csum,
	.set_flags		= lio_set_flags,
	...
	.get_module_info	= lio_get_module_info,
	.get_module_eeprom	= lio_get_module_eeprom,
#endif  /* LINUX_VERSION_CODE >= KERNEL_VERSION(3, 5, 0) */
};
...
static const struct ethtool_ops lio_vf_ethtool_ops = {
	...
	.get_coalesce		= lio_get_intr_coalesce,
	.set_coalesce		= lio_set_intr_coalesce,
	.get_priv_flags		= lio_vf_get_priv_flags,
	.set_priv_flags		= lio_vf_set_priv_flags,
#if LINUX_VERSION_CODE < KERNEL_VERSION(3, 0, 0)
	.get_tx_csum		= lio_get_tx_csum,
	.set_tx_csum		= lio_set_tx_csum,
	.get_rx_csum		= lio_get_rx_csum,
...
#if (RHEL_RELEASE_CODE > RHEL_RELEASE_VERSION(6, 3)) && (RHEL_RELEASE_CODE < RHEL_RELEASE_VERSION(7, 0))
static const struct ethtool_ops_ext lio_ethtool_ops_ext = {
...
static const struct ethtool_ops_ext lio_vf_ethtool_ops_ext = {
	.size		= sizeof(struct ethtool_ops_ext),
	.get_channels	= lio_ethtool_get_channels,
	.set_channels	= lio_ethtool_set_channels,
};
...
void liquidio_set_ethtool_ops(struct net_device *netdev)
{
	...
	if (OCTEON_CN23XX_VF(oct))
		...
		SET_ETHTOOL_OPS(netdev, &lio_vf_ethtool_ops);
#else
		netdev->ethtool_ops = &lio_vf_ethtool_ops;
#endif
	else
#if LINUX_VERSION_CODE < KERNEL_VERSION(3, 16, 0)
		SET_ETHTOOL_OPS(netdev, &lio_ethtool_ops);
#else
		netdev->ethtool_ops = &lio_ethtool_ops;
#endif
#if (RHEL_RELEASE_CODE > RHEL_RELEASE_VERSION(6, 3)) && (RHEL_RELEASE_CODE < RHEL_RELEASE_VERSION(7, 0))
	if (!OCTEON_CN23XX_VF(oct))
		set_ethtool_ops_ext(netdev, &lio_ethtool_ops_ext);
	else
		set_ethtool_ops_ext(netdev, &lio_vf_ethtool_ops_ext);
#endif
}
...
int liquidio_set_fec(struct lio *lio, int on_off)
{
	...
	if (oct->props[lio->ifidx].fec == on_off) {
		lio_dev_info(oct, "%s: SET FEC old %x new %x\n", __func__, oct->props[lio->ifidx].fec, on_off);
		return 0;
	}

	if (!OCTEON_CN23XX_PF(oct)) {
		lio_dev_err(oct, "%s: SET FEC only for PF\n", __func__);
		return -1;
		
	}

	if (oct->speed_boot != 25)  {
		lio_dev_err(oct, "%s: SET FEC only when link speed is set to 25G during insmod, link_speed_boot=%d\n", __func__, oct->speed_boot);	
		return -1;
	}
	...
	ncmd->s.param1 = on_off; //SEAPI_CMD_FEC_DISABLE(0) or SEAPI_CMD_FEC_RS(1)
	...
	retval = octeon_send_soft_command(oct, sc);
	if (retval == IQ_SEND_FAILED) {
		lio_info(lio, rx_err, "%s: Failed to send soft command\n", __FUNCTION__);
		octeon_free_soft_command(oct, sc);
		return -EIO;
	} else {
		/* Sleep on a wait queue till the cond flag indicates that the
		...
		if ((retval = cavium_sleep_cond_timeout(oct, sc, 0))) {
			...
			u32 var;
			var = cavium_be32_to_cpu(resp->fec_setting);
			resp->fec_setting = var;
			if (var != on_off) {
				lio_dev_err(oct, "%s: setting failed fec= %x, expect %x\n", __FUNCTION__, var, on_off);
				oct->props[lio->ifidx].fec = var;	
				if (resp->fec_setting == SEAPI_CMD_FEC_SET_RS) {
					oct->props[lio->ifidx].fec = 1;
					lio->oct_dev->priv_flags =
						lio->oct_dev->priv_flags | (0x1 << OCT_PRIV_FLAG_FEC);
				} else {
	...
	if (oct->props[lio->ifidx].fec != oct->props[lio->ifidx].fec_boot) {
		lio_dev_dbg(oct, "Reloade driver to chang fec to %s\n", oct->props[lio->ifidx].fec?"on":"off");
	}
...
int liquidio_set_speed(struct lio *lio, int speed)
{
	...
	if (oct->speed_setting == speed) {
		lio_dev_info(oct, "%s: no op: SET FEC old %x new %x\n", __func__, oct->speed_setting, speed);
		return 0;
	}

	if (!OCTEON_CN23XX_PF(oct)) {
		lio_dev_err(oct, "%s: SET SPEED only for PF\n", __func__);
		return -1;
	...
	ncmd->s.param1 = speed; //SEAPI_CMD_FEC_DISABLE(0) or SEAPI_CMD_FEC_RS(1)
	...
	if (retval == IQ_SEND_FAILED) {
		...
		if ((retval = cavium_sleep_cond_timeout(oct, sc, 0))) {
			...
			u32 var;
			var = cavium_be32_to_cpu(resp->speed);
			resp->speed = var;
			if (var != speed) {
				lio_dev_err(oct, "%s: setting failed speed= %x, expect %x\n", __FUNCTION__, var, speed);
			}
			
			oct->speed_setting = var;
			if (oct->speed_boot != oct->speed_setting) {
				lio_dev_dbg(oct, "Reload the driver to change adapter to %dG; The port is at %dG now\n", oct->speed_setting, oct->speed_boot);
			}
		}
	}
	...
	if (retval == IQ_SEND_FAILED) {
		...
		if ((retval = cavium_sleep_cond_timeout(oct, sc, 0))) {
			...
			if (var == 0xffff) {
				...
				lio_dev_dbg(oct, "%s: subsystem_id=%x, default speed: %dG\n",
					__FUNCTION__, oct->subsystem_id, oct->speed_setting);
			}
			if (oct->speed_boot != oct->speed_setting) {
				lio_dev_dbg(oct, "Reload the driver to change adapter to %dG; The port is at %dG now\n", oct->speed_setting, oct->speed_boot);
			}
